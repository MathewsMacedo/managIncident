  ...
  <table class="table table-bordered table-hover table-responsive">
    ...
    {{ if compare $incident.DateResolution "0001-01-01 00:00:00 +0000 UTC"}}
      ...
      {{ else }}
        warning
      {{ end }}
      {{ else if ge (date $incident.DateResolution "d-m-Y H:i:s") (date $.dateNow "d-m-Y H:i:s")}}
        warning
      {{ else if compare $incident.ConfirmUser "1"}}
        active
      {{ else }}
        success
      {{ end }}'
    >
      ...
      <td>{{ date $incident.DateRequest "d-m-Y H:i" }}</td>
        ...
        {{ if compare $incident.DateResolution "0001-01-01 00:00:00 +0000 UTC"}}
          {{ if compare $incident.DateEstimated "0001-01-01 00:00:00 +0000 UTC"}}
            <td>Pas encore pris en compte</td>
          {{ else }}
